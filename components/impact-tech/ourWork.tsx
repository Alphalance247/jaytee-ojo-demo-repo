...
import { useInsideOurWork } from "@/store/impact-tech/homepage/InsideOurWorkStore";
import LoadingState from "./common/LoadingState";
const OurWork = () => {
  const [currentSlide, setCurrentSlide] = useState(0);
  const { images, loading, error, fetchImages } = useInsideOurWork();
  useEffect(() => {
    fetchImages();
  }, [fetchImages]);
  // const slides = [
  ...
  const imageSrc = slides[currentSlide]?.attachments?.[0]?.proxyUrl || "";
  return (
    <div className="w-full bg-white px-4 py-12 sm:py-16 md:py-20">
      <div className="max-w-[1222px] mx-auto">
        ...
        {loading ? (
          <div>
            <LoadingState className="my-6 h-[400px] mx-[50px]" />
          </div>
        ) : error ? (
          <div className="text-center text-red-500">{error}</div>
        ) : !loading && slides.length === 0 ? (
          <div>Slides not available</div>
        ) : (
          <>
